...
class Venue(db.Model):
    ...
    genre = db.relationship('VenueGenreList', backref='genres', lazy=True)

    def __repr__(self):
        return f'<Venue id: {self.id}, name: {self.name}, seeking_talent: {self.seeking_talent}'
...
class Artist(db.Model):
    ...
    genre = db.relationship('ArtistGenreList', backref='genres', lazy=True)
...
def is_upcoming(date):
    curr_date = datetime.datetime.now().strftime('%a %m, %d, %Y %H:%M%p')
    if date > curr_date:
        return True
    else:
        return False
...
def venues():
    ...
    try:
        ...
        venue = Venue.query.filter(Venue.name.like(search)).all()
    except:
        db.session.rollback()
        error = False
        print(sys.exc_info())
...
def show_venue(venue_id):
    ...
    try:
        ...
        for show in shows:
            if is_upcoming(str(show.start_time)):
